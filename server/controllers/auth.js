...
const startSession = async (userID, res) => {
    ...
    await dbPoolSync
        .query(`INSERT 
            INTO sessions (s_id, s_user_id, s_key) 
            VALUES ("${id}", "${userID}", "${sessionKey}")`)
    //отправка токена на клиент
    res.cookie(
        "token",
        `${head}.${payload}.${signature}`,
        {secure:true, httpOnly: true, sameSite: "none", maxAge: 31536000000}
    )
...
export const auth = (req, res, next) => {
    if (req.cookies['token']) {
        ...
        dbPool.query(`SELECT s_key, s_user_id FROM sessions WHERE s_id="${session.id}"`,
            (err, sessionInfo) => {
            if (sessionInfo[0]) {
                const signature = crypto
                ...
                if (signature === tokenParts[2])
                    dbPool.query(`SELECT u_id, u_name FROM users WHERE u_id="${sessionInfo[0]["s_user_id"]}"`,
                        (err, result) => {
                        req.user = result[0]
                        next()
                    })
